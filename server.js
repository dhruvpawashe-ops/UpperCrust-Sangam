...
app.post('/api/import/portfolio', auth, adminOnly, upload.single('file'), async (req, res) => {
  ...
  if (content.includes('<') || content.includes('html') || content.includes('table')) {
    ...
    for (const row of rowMatches) {
      ...
      if (!headerRow && cells.includes('AMC') && cells.includes('Investor')) { headerRow = cells; continue; }
      if (headerRow && cells.length >= headerRow.length - 2 && cells[0] && !isNaN(parseInt(cells[0]))) {
        const obj = {};
