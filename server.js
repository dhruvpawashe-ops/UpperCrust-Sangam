...
app.post('/api/import/investors', auth, adminOnly, upload.single('file'), async (req, res) => {
  ...
  // Upsert by PAN (or name+rm_key if no PAN)
  let inserted = 0;
  for (let i = 0; i < rows.length; i += 200) {
    const batch = rows.slice(i, i + 200);
    const withPAN = batch.filter(r => r.pan);
    const withoutPAN = batch.filter(r => !r.pan);
    if (withPAN.length) {
      const { error } = await supabase.from('clients').upsert(withPAN, { onConflict: 'pan' });
      if (!error) inserted += withPAN.length;
      else console.error('Upsert error (with PAN):', error.message);
    }
    if (withoutPAN.length) {
      const { error } = await supabase.from('clients').insert(withoutPAN);
      if (!error) inserted += withoutPAN.length;
    }
  }
